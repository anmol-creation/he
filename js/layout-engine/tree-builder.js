export function buildTree(rawData, nodesMap, transitionWires) {
    ...
    rawData.forEach(originalNode => {
        ...
        // --- NEW RULES INJECTION ---
        // 1. If male, strip spouseOf so he doesn't auto-duplicate as a wife
        if (node.gender === 'male' && node.spouseOf) {
            node.spouseOf = null;
        }
        // 2. Use parent_id_a as primary drawing parent if it exists
        if (node.parent_id_a) {
            node.parent = node.parent_id_a;
        }
        // ---------------------------
    ...
    nodesMap.forEach(node => {
        ...
        if (node.parent && (!node.spouseOf || node.isCluster)) {
             if (nodesMap.has(node.parent)) {
                ...
                // MULTIPLE WIVES DYNAMIC ROUTING RULE
                const hasMultipleWives = parentNode.spouses && parentNode.spouses.length > 1;
                const alternativeParent = node.parent_id_b || node.mother; // fallback to mother for old data

                if (alternativeParent && nodesMap.has(alternativeParent) && hasMultipleWives) {
                    const altNodeRaw = rawData.find(d => d.id === alternativeParent);
                    if (altNodeRaw) {
                        // Check if the alternative parent is hidden inside a closed cluster
                        const isAltClustered = altNodeRaw.clusterName && !expandedClusters.has(altNodeRaw.clusterName);

                        if (!isAltClustered) {
                            // If she is visible on the canvas, route the line from her
                            nodesMap.get(alternativeParent).children.push(node.id);
                            pushedToMother = true;
                        }
                    }
                }
